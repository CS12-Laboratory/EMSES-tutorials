...
`dshield*` では `plasma.toml` を標準の入力ファイルとして使う。`[meta.physical]` を編集した場合は、投入前に以下を実行する。
...
`plasma.toml` があれば `[mpi].nodes` を参照し、無ければ legacy な `plasma.inp` の `nodes(:)` を参照して job.sh にプロセス数を設定し `sbatch job.sh` を実行する
```
...
`dshield0/plasma.toml`を見ると、`nstep`が`10`となっている。
...
`dshield1`や`dshield2`についても、パラメータファイル`plasma.toml`を見て、実行してみる

`[meta.physical]` を編集したときは、再投入前に `emu apply plasma.toml` を忘れずに実行すること
...
> [!NOTE]
> `ds0`で`wp = 0.0d0`のままだと、可視化ライブラリ`emout`が上手く機能しません。
>
> そのため、`ds0`を可視化する際には`plasma.toml`内の`[[species]]`の`wp`を一時的に`1.0d0`などへ変更してから可視化を行ってください。
...
上級例（旧 `advance/`）は、`MPIEMSES3D` 側の `parameter_examples` へ移行した。必要に応じてそちらを参照すること。
