...
`EMSES-tutorials` では `plasma.toml` を推奨形式に切り替えました。legacy な入力は各ケースの `.old/plasma.inp` / `.old/plasma.preinp` として隔離してあり、新規編集は `plasma.toml` を使ってください。
...
`MPIEMSES3D` の `inp2toml` を使えば、archive してある `.old/plasma.inp` を `format_version = 2` の `plasma.toml` へ変換できます。
...
`.old/plasma.preinp` に置いていた物理量ベースの入力は、`plasma.toml` の `[meta.physical]` と `emu apply` に移すのが現在の推奨です。
