...
`EMSES-tutorials` では `plasma.toml` を推奨形式に切り替えました。legacy な入力は `plasma.inp.old` / `plasma.preinp.old` として隔離してあり、新規編集は `plasma.toml` を使ってください。
...
`MPIEMSES3D` の `inp2toml` を使えば、archive してある `plasma.inp.old` を `format_version = 2` の `plasma.toml` へ変換できます。
...
`plasma.preinp.old` に置いていた物理量ベースの入力は、`plasma.toml` の `[meta.physical]` と `emu apply` に移すのが現在の推奨です。
