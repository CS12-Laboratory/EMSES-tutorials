...
`EMSES-tutorials` now treats `plasma.toml` as the recommended input format. `plasma.inp` is still kept in a few places for backward compatibility, but new edits should be made in `plasma.toml`.
...
Use `inp2toml` from `MPIEMSES3D` to convert `plasma.inp` into a `format_version = 2` `plasma.toml`.
...
If you previously relied on `plasma.preinp` for physical-unit input and unit conversion, the recommended replacement is `[meta.physical]` in `plasma.toml` together with `emu apply`.
